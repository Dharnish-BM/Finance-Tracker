...
function CalendarPage() {
  ...
  const events = payments.map(p => ({
    id: p.id,
    title: `${p.name} - ₹${p.amount} ${p.paid ? '(Paid)' : ''}`,
  ...
  return (
    <motion.div
      ...
      className="min-h-screen pt-24 px-4 lg:px-6 bg-gradient-to-br from-[#e0c3fc] to-[#8ec5fc] flex flex-col lg:flex-row gap-6 pb-6"
    >
      {/* Calendar */}
      <div className="w-full lg:w-[70%] flex-1 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-4 flex flex-col">
        <BigCalendar
          localizer={localizer}
          events={events}
          startAccessor="start"
          ...
          eventPropGetter={event => ({
            style: {
              backgroundColor: event.paid ? '#22c55e' : '#facc15',
              color: '#000',
              borderRadius: '0.5rem',
              border: 'none',
              padding: '2px',
      ...
      {/* Sidebar */}
      <div className="w-full lg:w-[25%] flex flex-col gap-6 h-[calc(100vh-4rem)]">
        {/* Payment List */}
        <div className="flex-1 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-4 flex flex-col">
          <h2 className="font-bold text-lg mb-4 flex justify-between items-center">
            Scheduled Payments
            <button onClick={()=>setModalOpen(true)} className="bg-blue-500 text-white px-3 py-1 rounded-lg text-sm hover:bg-blue-600">+ Add</button>
          </h2>
        ...
        {/* Progress */}
        <div className="h-32 bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-4 flex flex-col justify-center">
      ...
      {modalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
          <div className="bg-white rounded-2xl shadow-lg p-6 w-80 flex flex-col gap-4 animate-fade-in">
            ...
            <input type="text" name="name" placeholder="Bill Name" value={form.name} onChange={handleChange} className="border px-3 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-blue-400"/>
            <input type="number" name="amount" placeholder="Amount" value={form.amount} onChange={handleChange} className="border px-3 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-blue-400"/>
            <input type="date" name="dueDate" value={form.dueDate} onChange={handleChange} className="border px-3 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-blue-400"/>
            <select name="recurring" value={form.recurring} onChange={handleChange} className="border px-3 py-2 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-blue-400">
            ...
            <div className="flex justify-end gap-2 mt-2">
              <button onClick={()=>{setModalOpen(false); setEditPayment(null)}} className="px-4 py-2 rounded-lg border hover:bg-gray-100">Cancel</button>
              <button onClick={handleSubmit} className="px-4 py-2 rounded-lg bg-blue-500 text-white hover:bg-blue-600">Save</button>
            </div>
          </div>
        </div>
      )}
